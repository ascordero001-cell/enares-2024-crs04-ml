...
QUALITY_OUTPUT = Path(
    ...
    "pilot_32_household_quality.sqlx"
)

PARITY_OUTPUT = Path(
    "dataform/definitions/assertions/"
    "pilot_32_household_v0_parity.sqlx"
)
...
def detail_condition(index: int) -> str:
    return f"""CASE
      WHEN
        ...
        AND C3P203 = 1
        AND (
          C3P201A_{index} IN (1, 2, 3, 4, 19)
          OR C3P201E_{index} IN (1, 2, 3, 4, 19)
          OR (
            C3P201A_{index} IS NOT NULL
            AND C3P201A_{index} NOT IN (1, 2, 3, 4, 19)
            AND C3P201C_{index} = 1
            AND C3P201D_{index} = 1
          )
          OR (
            C3P201E_{index} IS NOT NULL
            AND C3P201E_{index} NOT IN (1, 2, 3, 4, 19)
            AND C3P201F_{index} = 1
          )
        )
      THEN 1
      ELSE 0
...
model = f"""config {{
  ...
  name: "analytical_crs04_pilot_32_household_v0_5",
  description: "Stage 03 shadow pilot for SPSS VP_HOGAR.",
  tags: ["stage03", "analytical", "pilot_32", "shadow"]
}}
...
quality = """config {
  ...
  tags: ["stage03", "analytical", "pilot_32"]
}

WITH
candidate AS (
  SELECT *
  FROM ${ref("analytical_crs04_pilot_32_household_v0_5")}
),
...
checks AS (
  ...
  SELECT
    ...
    COUNTIF(VP_HOGAR NOT IN (0, 1) OR VP_HOGAR IS NULL)
  FROM candidate
  HAVING COUNTIF(
    VP_HOGAR NOT IN (0, 1) OR VP_HOGAR IS NULL
  ) > 0
)
...
parity = """config {
  type: "assertion",
  schema: "enares2024_crs04_ops",
  tags: ["stage03", "analytical", "pilot_32", "parity"]
}
