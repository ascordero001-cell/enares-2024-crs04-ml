...
REFERENCE_SUFFIX = " [referencial]"

VS_MATRIX_SOURCE_CATEGORIES = (
...
VS_MATRIX_CATEGORIES = tuple(
    category.removesuffix(REFERENCE_SUFFIX) for category in VS_MATRIX_SOURCE_CATEGORIES
)
VS_MATRIX_PAIRS = frozenset(
    (("2×2", category) if index < 2 else ("3×3", category))
    for index, category in enumerate(VS_MATRIX_CATEGORIES)
)
VS_MATRIX_SOURCE_PAIRS = frozenset(
    (("2×2", category) if index < 2 else ("3×3", category))
    for index, category in enumerate(VS_MATRIX_SOURCE_CATEGORIES)
)
...
def normalize_vs_matrix_category(category: str) -> str:
    """Remove the source's presentation marker from the category identity."""
    return category.removesuffix(REFERENCE_SUFFIX)
